package com.zibanchegueman.flow
...
class IOUFlowTests {
    ...
    fun setup() {
        ...
        listOf(a, b).forEach { it.registerInitiatedFlow(IOUFlows.Acceptor::class.java) }
        network.runNetwork()
    }
    ...
    fun `flow rejects invalid IOUs`() {
        val flow = IOUFlows.IOUFlow(a.info.singleIdentity(), b.info.singleIdentity(),"sejong","GS25",-1,"20191111")
    ...
    fun `SignedTransaction returned by the flow is signed by the initiator`() {
        val flow = IOUFlows.IOUFlow(a.info.singleIdentity(), b.info.singleIdentity(),"sejong","GS25",10000,"20191111")
        val future = a.startFlow(flow)
        network.runNetwork()
    ...
    fun `recorded transaction has no inputs and a single output, the input IOU`() {
        val iouValue = 1
        val flow = IOUFlows.IOUFlow(a.info.singleIdentity(), b.info.singleIdentity(),"sejong","GS25",iouValue,"20191111")
        val future = a.startFlow(flow)
        network.runNetwork()
        val signedTx = future.getOrThrow()
        ...
        for (node in listOf(a, b)) {
            ...
            assertEquals(recordedState.affiliation, a.info.singleIdentity())
            assertEquals(recordedState.vaultServer, b.info.singleIdentity())
        }
    }
