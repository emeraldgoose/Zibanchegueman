...
class ServerApi(private val rpcOps: CordaRPCOps) {
    ...
    @Path("create-iou")
    fun createIOU(
            @QueryParam("peer") peer: CordaX500Name?, @QueryParam("from") from: String, @QueryParam("to") to: String, @QueryParam("amount") amount: Int, @QueryParam("date") date: String): Response {
        ...
        val otherParty = rpcOps.wellKnownPartyFromX500Name(peer) ?:
                return Response.status(BAD_REQUEST).entity("Party named $peer cannot be found.\n").build()

        return try {
            val signedTx = rpcOps.startTrackedFlow(::IOUFlow, otherParty, from, to, amount, date).returnValue.getOrThrow()
