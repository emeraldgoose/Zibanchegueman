...
class ServerApi(private val rpcOps: CordaRPCOps) {
    ...
    @Path("create-iou")
    fun createIOU(
            @QueryParam("peer") peer: CordaX500Name?, @QueryParam("from") from: String, @QueryParam("to") to: String, @QueryParam("value") value: Int, @QueryParam("date") date: String): Response {
        ...
        return try {
            val signedTx = rpcOps.startTrackedFlow(::IOUFlow, otherParty, from, to, value, date).returnValue.getOrThrow()
